...
- `proposed-v3/` — a draft next version of the assessment with its own `src/` and `answers/`. Treat as a parallel work-in-progress; do not conflate with the live `src/`.
...
- `proposed-v3/` mirrors the top-level structure (`src/` + `answers/`). Changes there are proposals and should not be cross-applied to the top-level `src/` unless explicitly requested.
